...
Beets::Beets(const sf::Vector2f& position, float waitTime, float SellValue)
    : position(position),
...
void Beets::setGrowthTime(float waitTime) { this->waitTime = waitTime; }

void Beets::setSellValue(float sellValue) { this->sellValue = sellValue; }

float Beets::getSellValue() { return sellValue; }

std::string Beets::getCropType() { return "Beets"; }