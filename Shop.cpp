...
std::string Shop::getItemVarStr(const std::string& itemName, const std::string& variable) {
    if (variable == "Price") {
        std::ostringstream fertPriceStream;
        fertPriceStream << std::fixed << std::setprecision(2) << itemPrices[itemName]; // Format to two decimal places
        std::string fertPriceString = fertPriceStream.str(); // Get the formatted string
        return fertPriceString;
    } else {
        std::ostringstream workerPriceStream;
        workerPriceStream << std::fixed << std::setprecision(2) << itemModifier[itemName];
...
int Shop::getCropVar(const std::string& cropName, const std::string& variable) {
    if (variable == "Price") {
        ...
        return cropGrowthTime[cropName] * weatherModifier["Rainfall"] * weatherModifier["Temperature"];
    } else {
        return cropSellValue[cropName];
    }
}

std::string Shop::getCropVarStr(const std::string& cropName, const std::string& variable) {
    if (variable == "Price") {
        ...
        growthStream << std::fixed << std::setprecision(2) << cropGrowthTime[cropName] * weatherModifier["Rainfall"] * weatherModifier["Temperature"];
...
void Shop::weatherAffected(float temperature, float rainfall) {
    ...
    if (temperature > 30.0f) { // If too hot decrease growth rate
        temporaryTemperatureModifier = ((temperature + 3) - 30.0f) * 0.5; // Add 3 to make sure growth rate is above 1
    } else if (temperature < 15.0f) { // If too cold decrease growth rate
        temporaryTemperatureModifier = (15.0f - (temperature - 3)) * 0.5; // Subtract 3 to make sure growth is above 1
    }
...
std::string Shop::getWeatherModfiersStr(const std::string& cropName) {
    ...
    weatherStream << std::fixed << std::setprecision(2) << weatherModifier[cropName];
