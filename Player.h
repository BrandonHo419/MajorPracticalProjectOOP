...
class Player {
 private:
  enum class indexAnimation { Upwards, UpwardsIdle, Downards, DownwardsIdle, Left, LeftIdle, Right, RightIdle, Count }; // Stores animations

 public:
  Player(const sf::Vector2f& position); // Initialises the players position
  void Draw(sf::RenderTarget& rt) const; // Draws the player
  void SetDirection(const sf::Vector2f& direction); // Sets which direction the player is facing
  void Update(float dt, float waitTime, bool resetFrame); // Updates the animation of the player every frame
  sf::Vector2f getCharPosition(); // Gets the player's/character position
  void setCharPosition(float x, float y); // Sets the player's/character position
