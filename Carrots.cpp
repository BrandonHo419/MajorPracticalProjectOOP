...
Carrots::Carrots(const sf::Vector2f& position, float waitTime, float SellValue)
    : position(position),
...
void Carrots::setGrowthTime(float waitTime) { this->waitTime = waitTime; }

void Carrots::setSellValue(float sellValue) { this->sellValue = sellValue; }

float Carrots::getSellValue() { return sellValue; }

std::string Carrots::getCropType() { return "Carrots"; }